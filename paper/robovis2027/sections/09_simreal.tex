...
It is therefore the one axis on which a simulated and a physical run
measure the same signal with the same instrument, and it is the only
axis this paper compares numerically. Everything spatial is excluded by
construction, because the physical runs have no external reference
(Section~\ref{sec:realresults}).

We state plainly what this comparison is not. The frozen simulated stack
and the flown stack differ in the eleven ways of
Table~\ref{tab:gap}, so agreement here would not establish that the
calibration worked and disagreement would not establish that it failed.
The question is narrower: on the axis where both domains measure the same
thing, how far apart are they, and does the distance move with the
calibration level?
...
Commitment distance is reported in the frozen study and is absent from
the physical results, and the reason is not merely missing
instrumentation. The physical vehicle started \SI{1.86}{\metre} from the
anchor with an engagement range of \SI{1.80}{\metre}, so it was inside
engagement range at release; three of the four \textsf{K0} runs are
already commanding laterally at their first recorded sample. Even with
perfect tracking, physical commitment distance would have been a
property of where the vehicle was put, not of what the planner decided.
Reporting a sim-real error on that quantity would compare a measurement
against a geometric certainty, and we do not report one.
