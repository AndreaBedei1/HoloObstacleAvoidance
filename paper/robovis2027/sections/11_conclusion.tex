...
test. The stack did not transfer. Making it work took seven documented
changes, and the two that shaped the physical experiment most were not
calibrated quantities at all: the acceptance gates of the fallback
detector confined it to an observable range window of
\SIrange{0.29}{2.11}{\metre}, so that a vehicle released
\SI{1.86}{\metre} from the anchor with an engagement range of
\SI{1.80}{\metre} had no approach phase; and the planner's manoeuvre
asks for a \SI{2.5}{\metre} lateral offset in a basin
\SIrange{2.7}{3.0}{\metre} wide.
...
whether the system worked in the pool. What determined it was the
interaction between a perception module's acceptance criteria and the
size of the room.
