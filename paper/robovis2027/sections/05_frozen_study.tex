...
repetitions. \emph{cl}: minimum clearance (\si{\metre}); \emph{cmt}:
commitment distance (\si{\metre}); \emph{lat}: maximum lateral excursion
(\si{\metre}); \emph{span}: manoeuvre span (\si{\second}); \emph{qual}:
range at first qualified observation (\si{\metre}). The dynamic-window
commitment distance equals the starting distance at every level: it
strafes from the first cycle, so the value is a left-censored bound and
not a measurement.}
\label{tab:frozen}
\setlength{\tabcolsep}{4.2pt}
\begin{tabular}{llrrrrrrrrrrrrrrrrrrrr}
\toprule
& & \multicolumn{4}{c}{cl} & \multicolumn{4}{c}{cmt}
& \multicolumn{4}{c}{lat} & \multicolumn{4}{c}{span}
& \multicolumn{4}{c}{qual} \\
\cmidrule(lr){3-6}\cmidrule(lr){7-10}\cmidrule(lr){11-14}%
\cmidrule(lr){15-18}\cmidrule(lr){19-22}
